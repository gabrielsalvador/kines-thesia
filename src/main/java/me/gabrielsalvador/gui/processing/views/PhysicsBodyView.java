...
import me.gabrielsalvador.core.PhysicsBodyComponent;
    ...
    public void display(PGraphics graphics, Component model) {
        ...
        switch (shape.getType()) {
            case CIRCLE:
                ...
                Vec2 pixelPosition = MathUtils.coordWorldToPixels(position);
            ...
            case POLYGON:
                ...
                graphics.pushMatrix();
                pixelPosition = this.model.getPosition();
                ...
                for (int i = 0; i < 4 ; i++) {
                    ...
                    Vec2 pixelVertex = MathUtils.coordWorldToPixels(vertex.x, vertex.y);
...
//                graphics.popMatrix();
    ...
    public boolean isMouseOver(int mouseX, int mouseY) {
        ...
        Shape shape = body.getFixtureList().getShape();
        Vec2 pixelPosition = MathUtils.coordWorldToPixels(this.model.getPosition());
        Vec2 bodyPixelPos = MathUtils.coordWorldToPixels(body.getPosition().x, body.getPosition().y).add(pixelPosition);
    ...
    private boolean isMouseOverPolygon(int mouseX, int mouseY, PolygonShape polygon, float angle, Vec2 bodyPos) {
        ...
        for (int i = 0, j = polygon.getVertexCount() - 1; i < polygon.getVertexCount(); j = i++) {
            ...
            viPixelSpace = MathUtils.coordWorldToPixels(vertexI.x, vertexI.y);
            vjPixelSpace = MathUtils.coordWorldToPixels(vertexJ.x, vertexJ.y);
